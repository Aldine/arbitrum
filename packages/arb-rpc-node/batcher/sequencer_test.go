...
func TestSequencerBatcher(t *testing.T) {
	...
	_, err = seqMon.StartInboxReader(ctx, client, common.NewAddressFromEth(rollupAddr), rollupBlock.Int64(), common.NewAddressFromEth(bridgeUtilsAddr), dummySequencerFeed)
	test.FailIfError(t, err)

	_, err = otherMon.StartInboxReader(ctx, client, common.NewAddressFromEth(rollupAddr), rollupBlock.Int64(), common.NewAddressFromEth(bridgeUtilsAddr), dummySequencerFeed)
	test.FailIfError(t, err)
